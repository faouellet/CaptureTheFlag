...
Planner::Actions Planner::GetNextAction(const BotInfo* in_Bot, const Planner::State & in_CurrentState, 
										const float in_CurrentTime, const std::vector<MatchCombatEvent> & in_Events)
{
#ifdef _TRAIN
	ActionState l_PreviousActionState = m_LastOrders[in_Bot->name];
	double l_Reward = ComputeReward(in_Bot, in_CurrentTime, l_PreviousActionState.first, in_Events);
	...
	{
		l_Action = static_cast<Actions>(m_RandomGenerator() % 7);
	}
	else
	{
...
int Planner::ComputeReward(const BotInfo * in_Bot, const float in_CurrentTime,
				  const Planner::Actions in_PreviousAction, const std::vector<MatchCombatEvent> & in_Events)
{
	int l_Reward = 0;

	auto l_KilledIt = std::find_if(in_Events.begin(), in_Events.end(), [&in_Bot, &in_CurrentTime](const MatchCombatEvent & in_Event)
			{
				return in_Event.type == MatchCombatEvent::TYPE_KILLED
					&& std::abs(in_CurrentTime - in_Event.time) < 0.00001;
			});

	if(l_KilledIt != in_Events.end() && l_KilledIt->killedEventData.subject == in_Bot)
		l_Reward -= 100;

	auto l_EventIt = std::find_if(in_Events.begin(), in_Events.end(), [&in_CurrentTime](const MatchCombatEvent & in_Event)
			{
				return (in_Event.type == MatchCombatEvent::TYPE_FLAG_CAPTURED 
					|| in_Event.type == MatchCombatEvent::TYPE_FLAG_PICKEDUP)
					&& std::abs(in_CurrentTime - in_Event.time) < 0.00001;
			});

	if(l_EventIt != in_Events.end() && l_EventIt->type == MatchCombatEvent::TYPE_FLAG_CAPTURED)
	{
		if(l_EventIt->flagCapturedEventData.instigator->team == in_Bot->team)
			l_Reward += 1600;
		else
			l_Reward -= 400;
	}
	
	switch(in_PreviousAction)
	{
		case GetEnemyFlag:
		{
		...
		case ReturnToBase:
		{
			if(l_EventIt != in_Events.end() && l_EventIt->type == MatchCombatEvent::TYPE_FLAG_CAPTURED && l_EventIt->flagCapturedEventData.subject != in_Bot->team->flag)
			{
				l_Reward += 320;
			}
			else if(in_Bot->flag)
